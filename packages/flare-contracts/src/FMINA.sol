...
/// and FMINA uses exactly the same base unit, so one nanomina locked on Mina is
/// one FMINA base unit minted here. The collateral invariant
...
contract FMINA is ERC20, ERC20Permit {
    ...
    constructor(address bridge) ERC20("Flare MINA", "FMINA") ERC20Permit("Flare MINA") {
    ...
    /// @notice Mint FMINA against newly escrowed MINA. Bridge only.
    /// @dev Emits the standard `Transfer(address(0), to, amount)`.
    function mint(address to, uint256 amount) external onlyBridge {
        _mint(to, amount);
    }

    /// @notice Burn FMINA being withdrawn to Mina. Bridge only.
